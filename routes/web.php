...
Route::post('/create-multipart-upload', function (Request $request) {
    $meta = $request->get('meta');
    $project = Project::find($meta['projectId']);
    if (!$project) {
...
Route::post('/prepare-upload-part', function (Request $request) {
    $sharedConfig = [
        'region'  => config('filesystems.disks.s3.region'),
        'version' => 'latest',
        'credentials' => [
...
Route::post('/list-parts', function (Request $request) {
    $sharedConfig = [
        'region'  => config('filesystems.disks.s3.region'),
        'version' => 'latest',
        'credentials' => [
...
Route::post('/abort-multipart-upload', function (Request $request) {
...
Route::post('/complete-multipart-upload', function (Request $request) {
    $sharedConfig = [
        'region'  => config('filesystems.disks.s3.region'),
        'version' => 'latest',
        'credentials' => [
